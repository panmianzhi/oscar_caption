...
from .origin_bert.modeling_bert import BertPreTrainedModel, BertEmbeddings, BertPooler, BertLayerNorm,\
    BertLayer, BertEncoder, BertSelfAttention, BertAttention, BertSelfOutput, BertOutput, BertIntermediate, \
    BertOnlyMLMHead
from .caption_model_utils import CaptionPreTrainedModel, ConstrainedBeamSearch, select_best_beam_with_constraints
...
class BertCaptioningLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.label_smoothing = getattr(config, 'label_smoothing', 0) # default 0
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.bert = BertImgModel(config) # return [b, seq_len, hid_size]
        self.cls = BertOnlyMLMHead(config) # [b, seq_len, hid_size] -> [b, seq_len, vocab_size]
        self.loss = BertCaptioningLoss(config)

        self.apply(self.init_weights)
        self.tie_weights() # clone embedding layer's weight to decode
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None,
        ...
        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask,
                ...
                head_mask=head_mask,
                encoder_history_states=encoder_history_states)
        sequence_output = outputs[0][:, :masked_pos.shape[-1], :] # [batch_size, seq_len, hid_dim]
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        '''
        :param curr_ids: (b * num_beams, cur_len) = (batch_size, cur_len)
        :param past:
        :return:
        '''
        # if attention is on, it should be the token used to mask words in training
        mask_token_id = self.mask_token_id
        batch_size = curr_ids.shape[0]
        mask_ids = torch.full(
            (batch_size, 1), mask_token_id, dtype=torch.long, device=curr_ids.device
        ) # (batch_size, 1) filled with [MASK] ids

        # following two funcs are for sequences
        ...
        if past is None: # panmz: first iteration
            # panmz: cur_ids is ['[BOS]']
            input_ids = torch.cat([curr_ids, mask_ids], dim=1) # (batch_size, cur_len + 1)

            curr_len = input_ids.shape[1] # panmz: 2
            full_len = self.max_seq_len + self.od_labels_len + self.img_seq_len
            assert self.full_attention_mask.shape == (batch_size,
                    full_len, full_len)

            # remove a set of cross-like elements from origin matrix
            ...
            attention_mask = _remove_rows_cols(self.full_attention_mask, seq_start,
                    seq_end, seq_start, seq_end) # (batch_size, od_labels_len + img_seq_len + curr_len,
                                                            #   od_lebels_leb + img_seq_len + curr_len)
            ...
            img_feats = self.img_feats # [batch_size, num_boxes, dim]
            ...
            input_ids = torch.cat([last_token, mask_ids], dim=1) # (batch_size, 2)
            start_pos = curr_ids.shape[1] - 1
            end_pos = start_pos + input_ids.shape[1] # curr_ids.shape[1] + 1
            masked_pos = _slice(self.full_masked_pos, start_pos, end_pos)
            token_type_ids = _slice(self.full_token_type_ids, start_pos, end_pos)
            position_ids = _slice(self.full_position_ids, start_pos, end_pos) # (batch_size, 2)

            img_feats = None
            assert past[0].shape[0] == batch_size
            if self.prev_encoded_layers is None: # panmz: second iteration, ['[BOS]', 'w1']
                assert start_pos == 1  # the first token after BOS
                assert past[0].shape[1] == 2 + self.od_labels_len + self.img_seq_len # token len after first iteration
                # reorder to [od_labels, previous sentence]
                ...
                i2i = self.full_attention_mask[:, self.max_seq_len:,
                        self.max_seq_len:]
                # panmz: actually s represents origin sentece, while
                # panmz: i represents 'od_labels + img_feat'
                self.full_attention_mask = torch.cat(
                        [torch.cat([i2i, i2s], dim=2),
                        torch.cat([s2i, s2s], dim=2)],
                        dim=1)
            else: # panmz: >= 3 iterations
                assert start_pos > 1
    ...
    # ATTENTION: use_cbs=False when inference on COCO test
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
        ...
        """ Generates captions given image features
            params:
                use_cbs: whether use ConstrainedBeamSearch
        ...
        if num_return_sequences != 1:
            ...
            input_ids = self._expand_for_beams(input_ids, num_return_sequences) # [b * num_return_seqs, seq_len]
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(0).expand([batch_size, posids_len]) # [batch_size, posids_len]
    ...
    def _expand_for_beams(self, x, num_expand):
        ...
        expanded_shape = input_shape[:1] + [num_expand] + input_shape[1:] # [batch_size, num_expand, len, ...]
        x = x.unsqueeze(1).expand(expanded_shape) # [batch_size, num_expand, len, ...]
